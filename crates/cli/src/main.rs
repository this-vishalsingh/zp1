...
use zp1_prover::{ProofConfig, SerializableProof, StarkConfig, StarkProver};
...
async fn prove_block_command(
    ...
    match BlockProver::new(config).await {
        Ok(mut prover) => match prover.prove_block(block_number).await {
            Ok(proof) => {
                ...
                println!(
                    "   Proof saved to: {}/block_{}.json",
                    output_dir, block_number
                );
            }
            Err(e) => {
...
async fn prove_blocks_command(
    ...
    match BlockProver::new(config).await {
        Ok(mut prover) => match prover.prove_block_range(from, to).await {
            Ok(proofs) => {
                let elapsed = start.elapsed();
...
fn verify_block_command(proof_path: &PathBuf) {
    ...
    println!(
        "🔍 Verifying Ethereum block proof: {}",
        proof_path.display()
    );
...
fn prove_command(
    ...
    let serializable = SerializableProof {
        ...
        query_proofs: proof
            .query_proofs
            .iter()
            .map(|qp| zp1_prover::serialize::SerializableQueryProof {
                index: qp.index,
...
fn bench_command(log_size: usize) {
    ...
    println!(
        ...
        trace_len as f64 / prove_time.as_secs_f64()
    );
    ...
    println!(
        "  Final poly:    {} coeffs",
        proof.fri_proof.final_poly.len()
    );
